...
**PD literature verification (completed Aug 2026) — THREE MORE CRITICAL FINDINGS:** (a) the drafted [13] anchor citation was WRONG — its DOI resolves to an unrelated paper and the cited TDEI 2019 title could not be found; the genuine source is Lusuardi/Rumi/Cavallini/Barater/Nuzzo, IEEE Access vol. 9, pp. 27485–27495, 2021 (now drafted as [13]); (b) companion [14] resolved: Meyer/Cavallini et al., TDEI 25(3):873–882, 2018 (+ Madonna et al., TIA 57(2), 2021 as alternative); (c) ⚠️ the core "200–300 V at quarter-atmosphere / 4–15× margin" numbers are NOT supported by the literature — supported wording: PDIV roughly halves from sea level to cruise (~0.7–1 kV → ~350–500 V peak), with the ~330 V Paschen-minimum floor pressure-independent. Re-derived, the claim is STRONGER (20–50 V is 6.6–16× below even the floor), but the S3 parameter table and every dependent text need a PI-approved sweep. Details in 06_References.md [13], [14] and Residual check 5.

**Gap-research outcomes (evidence/11–16, Aug 2026) — TWO MORE DECISIONS REQUIRED:** (d) ⚠️ NOVELTY REFRAME: ISCAD (Dajaku/Gerling, SAE 2016) already demonstrated per-slot excitation and electronic pole changing at 48 V/110 kW, and the Wisconsin GaN IMMD (TIA 2015) series-stacks converter modules to fraction the bus — the proposal's uniqueness claim survives only as the conjunction (bilateral open-end + magnetically derived safe voltages from the 270 V bus + monolithic IC + PD-elimination physics + adiabatic ZVS) and must cite both openly; differentiation table and ready fragments in evidence/14_per_slot_prior_art_research.md. (e) "5–50 kW per-motor class" in the Timeliness text is wrong for passenger eVTOL (Joby ~236 kW, Archer 120 kW×12); replacement wording in evidence/16. Also: gearbox and end-winding claims now have verified literature (evidence/12, 13); multiphase derating numbers secured (evidence/15); OEW canon secured (evidence/11); certification framing corrected to "partial, bounded, predictable failures" per SC-VTOL (evidence/16).
...
2. Decide the re-derived PD wording and margin factors (see warning above) → then sweep S3, Vision, Approach RQ3/H3, Summary.
3. Decide replacement market and jobs figures per the warning above.
4. Approve the Novelty reframe (d) and the power-class rewording (e) → then sweep Vision Novelty/Timeliness and References (four candidate entries from evidence/14, ~90 words).
